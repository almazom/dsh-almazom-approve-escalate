...
One click on the DSH web approval card: **approve the pending request and switch the live session to a permission preset** (default `workspace-write`) — instead of doing it in two steps.
...
- **Approve & escalate** — dispatches `/permission workspace-write` on the live session (the same command the shipped `/permission` picker uses, so the sandbox + approval policy switch applies to *this* session) and then answers the pending request as *Allow once*.
