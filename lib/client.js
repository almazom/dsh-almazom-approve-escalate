    ...
    function ApproveEscalate(props) {
      ...
      var onClick = useCallback(function () {
        ...
        escalate(ctx, sessionId, config.preset)
          .catch(function (err) {
            // Escalation failure does not answer the request — surface and stop.
            setNote(String(err && err.message ? err.message : err));
            setBusy(false);
            return undefined;
          })
          .then(function (escalated) {
            if (escalated === undefined) return; // escalation failed above
