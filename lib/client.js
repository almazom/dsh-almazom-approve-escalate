...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ApproveEscalate(props) {
      ...
      var fallbackNote = config.fromHost ? '' : ' (default — config unavailable)';
      var hint = config.preset === 'danger-full-access'
        ? 'switches this session to full access — sandbox off, no further approval prompts' + fallbackNote
        : 'also switches this session to ' + config.preset + fallbackNote;
